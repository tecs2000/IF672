...
void FRE(unordered_map<string, list<string>> &map, string &element, bool dlk_fre) {
    ...
    for (int i = 0; i < apagar.size(); i++) {                                       //Varre o array de chaves salvas
        ...
        if (oh == map[apagar[i]].begin()) {                                         //O processo esta usando o recurso
            ...
            if (map[apagar[i]].size() > 0) {                                        //Se ainda houverem processos na fila do recurso
                ...
                map[opa].remove(apagar[i]);                                         //Apaga o recurso dessa fila, pois agora ele ja esta sendo usado
            }
        } else {
            map[apagar[i]].erase(oh);
            filas++;
...
bool DLK_visiting(unordered_map<string, list<string>> &map, unordered_map<string, int> &colors, string &element) {
    ...
    if(element[0] == 'R')
        adjacentes.push_back(map[element].front());

    else adjacentes = map[element];                                 //Copia a sua lista de adjacencia
    ...
    while (comando != "END") {
        ...
        if (comando == "REQ") {
            ...
            if(so[recurso].size() == 1) cout << "AVAIL" << endl;
                ...
                cout << "WAIT " << so[recurso].size()-1 << endl;
                so[processo].push_back(recurso);
            }
        } else if (comando == "FRE")
